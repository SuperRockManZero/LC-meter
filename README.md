...
此專案是依據 LC 諧振原理，使用 PIC16F648A 微處理器製作的電感及電容數值測量表。

## 專案介紹

這是一個電子測量工具，用來精準測量電感$`(L)`$和電容$`(C)`$數值。該工作原理是利用 LC 諧振特性並測量其震盪頻率，根據已知的參考電容依諧振公式計算出待測量的電感或電容。並且增加了自動校準，可去除電路的寄生電容與電感及大部分元件可能造成的誤差。

## 特性
- 使用 PIC16F648A 微處理器
- 基於 LC 諧振原理測量頻率
- 根據已知的參考電容計算出電感或電容數值
- 自動校準可以有效去除寄生電容電感還有大部分元件所造成的誤差
- 測量範圍：
  - 電容：$`1pF \sim 0.97uF`$
  - 電感：$`100nH \sim 97mH`$
  - 誤差：$`\pm1\%`$

誤差值$`\pm1\%`$是取決於校準用電容的精密度。其它還有Mcu的石英震盪器誤差及浮點運算誤差，所以實際誤差將會稍大於$`\pm1\%`$。
...
#### **減少Mcu運算**：
$`formula (a2)`$、$`(b2)`$ 與 $`(c2)`$ 中的 $`T_{1}`$、$`ARG3`$與$`C`$，均在校準時可得知。為免去後續量測時重覆計算相同的運算，故將部份公式修改如下。
- 設常數：
  - $`ARG1={T_{1}}^{2}`$
  - $`ARG2=\frac{ARG3}{C}`$
